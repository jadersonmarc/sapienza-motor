...
/** Gera a imagem e sobe no bucket do tenant; retorna a URL pública (proxy), ou
 *  null se o storage não estiver configurado (seam — publica sem imagem). */
export async function generateAndStoreCover(tenantId: string, input: SocialImageInput): Promise<string | null> {
  if (!isStorageConfigured()) return null
  const { buffer, formatId } = await renderSocialImage(input)
  const key = r2KeyFor({ slug: input.slug, formatId })
  return uploadObject(tenantId, key, buffer, "image/png")
}
